...
`git-diff` 是 singleton tab。`artifact-file` 按 normalized artifact path 去重。同一个入口再次打开时激活已有 tab，并刷新该 tab 数据。
...
- `git-diff`：展示当前 git diff 详情。Diff 内容使用 `@git-diff-view/react` 渲染，按文件分组，显示 additions/deletions badge，并使用现有 shadcn `Badge` / `Separator` / `Button` 等组件保持视觉一致。
