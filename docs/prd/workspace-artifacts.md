...
把所有由 `edit` / `write` 工具改动到的 Markdown 文件识别为 Workspace Artifacts。Workspace Status Float 展示最近 artifacts；点击 artifact 会在 Right Panel 中打开 `artifact-file` tab，以只读方式查看文件内容。
...
Artifact v1 只包含 Markdown 文件：

- `.md`
- `.mdx`
- `.markdown`

只要成功的 `edit` 或 `write` 工具调用改动了这些文件，就将该文件视为 artifact。路径使用 workspace-relative normalized path 作为稳定 identity。
...
Artifacts 按 normalized path 去重。重复改动同一文件时更新 `updatedAt`、source tool 和 diff metadata，并把该文件移到列表前面。路径展示尽量使用 workspace-relative path；如果无法确认 workspace root，展示原始 path 的可读尾部。
...
点击 artifact 时，前端通过 WebSocket 方法请求文件内容。读取是只读操作，不触发 agent 工具调用，不修改 session。

## Out of Scope

- 不把非 Markdown 文件纳入 v1 Artifacts。
