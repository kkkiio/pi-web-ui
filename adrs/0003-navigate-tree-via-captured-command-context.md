...
我们将在 `/webui` 命令的 handler（运行在 `ExtensionCommandContext` 中）执行时**捕获 `navigateTree` 闭包**，存储为模块级变量，供 WebSocket handler 后续使用。
...
- 不修改 Pi 源码
- 不注册额外的公开斜杠命令（复用现有的 `/webui`）
- 命令对齐 Pi API：`navigate_tree` → `navigateTree`，`prompt` → `sendUserMessage`
- 两步操作让前端拥有编排权（navigate 成功后前端再发 prompt）

### 负向

- `/webui` 必须被执行过一次，`navigateTree` 才可用。在 `session_start`（fork/new/resume 等 session 替换后）需要清空，用户需重新运行 `/webui`
...
- Web UI 中 `/webui` 的文案从"Open browser"扩展为"Connect web UI for advanced features"
