...
export function SubagentDetailSidebar({
  ...
  return (
    ...
    >
      <div
        className="group absolute inset-y-0 left-0 z-20 hidden w-4 -translate-x-2 cursor-col-resize touch-none md:block"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        ...
        {agent.compactionCount ? (
          <div className="mb-4 flex items-center gap-2 rounded-md border bg-muted/40 px-3 py-2 text-muted-foreground text-sm">
            ...
            <span>{agent.compactionCount} compaction{agent.compactionCount === 1 ? '' : 's'}</span>
          </div>
        ) : null}
